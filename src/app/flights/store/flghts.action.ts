...
export const getFlights = createAsyncThunk<Paths[], GetFlights>("GET/flights", async (body, { rejectWithValue }) => {
    try {
        const response = await repository.get(`/flights?sortedBy=${body.sortedBy}&from_city=${body.from_city}&to_city=${body.to_city}&date=${body.start_flight_date}&isReturn=${body.isReturn}&returnDate=${body.return_flight_date}`);
